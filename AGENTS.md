...
## 3. 目录结构（解决方案 = `ExcelMerge.sln`）

```
ExcelMerge.sln                  # 解决方案（VS2015 格式，dotnet msbuild 可构建）
ExcelMerge\                      # 类库：工作簿/工作表/单元格读取、Diff 模型构建、CSV/TSV 解析
...
ExcelMerge.GUI\                  # WPF 可执行：UI、命令层、设置、IPC、托盘、本地化、常驻生命周期
...
依赖关系：`GUI → ExcelMerge → NetDiff`；`GUI → FastWpfGrid → WriteableBitmapEx.Wpf`。
...
```
dotnet msbuild ExcelMerge/ExcelMerge.csproj /p:Configuration=Release /p:TargetFrameworkRootPath="D:\ExcelMerge\packages\refs" /t:Build /v:m /nologo
```
...
| 需求 | 入口文件 | 说明 |
|------|---------|------|
| 新增文件类型解析 | `ExcelMerge/ExcelWorkbook.cs`、`CsvReader.cs`/`TsvReader.cs` | 按扩展名分发，EM/EME 都要过 |
| 差异算法调整 | `NetDiff/NetDiff/EditGraph.cs`、`DiffUtil.cs` | 行级/单元格级共用；改后跑 `NetDiff.Test` |
| 差异提取规则/日志格式 | `ExcelMerge/ExcelSheetDiff.cs`、`ExcelSheetDiffConfig.cs`、`DiffExtractionSettingWindow*` | |
...
- 命名空间 = 目录名（`ExcelMerge.GUI.ViewModels`、`ExcelMerge.GUI.Settings` 等）。
