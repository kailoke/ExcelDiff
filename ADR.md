...
## ADR-002 NPOI 是权威读取，EDR 仅性能验证

- **状态**：已定
- **背景**：EDR 更快，但读不到"仅样式无值"单元格，导致列对齐漂移、漏报真实差异。
- **决策**：EM=NPOI 权威；EDR 仅供 EME 测试版做性能/行为对照；EDR 路径尽力对齐 NPOI 语义（跳空行、裁尾空列）。
- **后果**：权威路线永远不可迁到 EDR（INVARIANT B1）；开发期用 `VerifyRead` 双读比对。
- **被否**：EDR 作为唯一读取器——会漏报差异，产品正确性不可接受。
