...
using ExcelDiff.GUI.Shell;

namespace ExcelDiff.GUI.Controls
{
	public class PowerShellConsole : RichTextBox
	{
		private InvocationSection _currentSection;
		...
		private void Invoke()
		{
			if (this._currentSection == null
				|| this._currentSection.Invocation.Status != InvocationStatus.Ready)
			{
				return;
			}

			var area = this._currentSection.Editor;
			var prompt = this._currentSection.Prompt;
			var script = new TextRange(area.ContentStart.GetPositionAtOffset(prompt.Length + 1), area.ContentEnd).Text;

			this._currentSection.Invocation.Script = script;
			this._currentSection.Invocation.Invoke();
		}
        ...
        protected override void OnPreviewKeyDown(KeyEventArgs e)
		{
			...
			else if (this.CaretIsInEditArea())
			{
				if (e.Key == Key.Escape)
				{
					this._currentSection.Invocation.Script = "";
					e.Handled = true;
				}
				if (e.Key == Key.Up)
				{
					this._currentSection.Invocation.SetNextHistory();
					e.Handled = true;
				}
		...
		private bool CaretIsInEditArea()
			=> this._currentSection != null && this._currentSection.CaretIsInEditArea(this.CaretPosition);

		private bool CaretIsInLeftMostOfEditArea()
			=> this._currentSection != null && this._currentSection.CaretIsInLeftMostOfEditArea(this.CaretPosition);
		...
		{
			private readonly PowerShellConsole _owner;
			...
			private void HandleInvocationPropertyChanged(object sender, PropertyChangedEventArgs args)
			{
				...
				switch (args.PropertyName)
				{
					case nameof(IPowerShellInvocation.Script):
						{
							if (Equals(this, this._owner._currentSection))
							{
								...
								range.Text = script;
								this._owner.CaretPosition = this.Editor.ContentEnd;
							}
							break;
						}
