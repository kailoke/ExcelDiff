# Invoke-ExcelDiffDiff.ps1 - Safe wrapper for starting a GUI diff.
#
...
function Wait-DiffWindow {
    ...
    while ((Get-Date) -lt $deadline) {
        $win = Get-Process -Name 'ExcelDiff*' -ErrorAction SilentlyContinue |
...
if (-not (Wait-DiffWindow -TimeoutSeconds $TimeoutSeconds)) {
    Write-Host 'ExcelDiff diff window did not appear in time' -ForegroundColor Red
    exit 1
}

if ($WaitClose) {
    if (-not (Wait-DiffWindow -UntilClosed -TimeoutSeconds $TimeoutSeconds)) {
        Write-Host 'ExcelDiff diff window did not close in time' -ForegroundColor Red
        exit 1
    }
}
