...
- [ ] **B4 回归基线**：EM/EME 输出比对对象必须是**同一文件的两个版本**（git HEAD vs 工作区），严禁拿两个不同文件互比。（AGENTS §7.4）
...
- [ ] **E6 本地构建命令**：必须传 `TargetFrameworkRootPath="D:\ExcelMerge\packages\refs"`（.NET Framework 引用程序集不在 SDK 里）。（AGENTS §4）
